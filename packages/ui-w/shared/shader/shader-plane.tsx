...
interface ShaderPlaneMeshProps {
  vertexShader: string
  fragmentShader: string
  uniforms: { [key: string]: { value: unknown } }
}

function ShaderPlaneMesh({ vertexShader, fragmentShader, uniforms }: ShaderPlaneMeshProps) {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      const mouseUniform = material.uniforms.u_mouse

      if (!timeUniform || !resolutionUniform || !mouseUniform) return

      timeUniform.value = state.clock.elapsedTime * 0.5
      resolutionUniform.value.set(size.width, size.height, 1.0)
      ...
      mouseUniform.value.set(mousePos.current.x, mousePos.current.y, 0.0, 0.0)
    }
  })
...
export function ShaderPlane({
  ...
  return (
    <section className={cn("relative h-svh max-h-300 min-h-150 w-full overflow-hidden", className)}>
      <Canvas>
        <ShaderPlaneMesh
          vertexShader={vertexShader}
          fragmentShader={fragmentShader}
