    ...
    <TooltipPrimitive.Portal {...portalProps}>
      ...
      >
        <TooltipPrimitive.Popup
          className={cn(
            "bg-popover text-popover-foreground border-border relative flex h-(--popup-height,auto) w-(--popup-width,auto) origin-(--transform-origin) rounded-xl border text-xs text-balance transition-[width,height,scale,opacity] not-dark:bg-clip-padding before:pointer-events-none before:absolute before:inset-0 before:rounded-xl data-ending-style:scale-98 data-ending-style:opacity-0 data-instant:duration-0 data-starting-style:scale-98 data-starting-style:opacity-0",
            className,
          )}
