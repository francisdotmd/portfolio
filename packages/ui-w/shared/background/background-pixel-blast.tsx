...
function resolveColor(element: HTMLElement, value: string) {
  ...
  return new THREE.Color(resolved || value)
}
...
export interface BackgroundPixelBlastProps {
...
export function BackgroundPixelBlast({
  variant = "square",
  pixelSize = 3,
  color = "#B497CF",
  className,
  ...
  useEffect(() => {
    ...
    const needsReinitKeys: Array<keyof PixelBlastConfig> = ["antialias", "liquid", "noiseAmount"]
    ...
    if (mustReinit) {
      ...
      const uniforms: PixelBlastUniforms = {
        ...
        uTime: { value: 0 },
        uColor: { value: resolveColor(container, color) },
        uClickPos: {
      ...
      t.uniforms.uColor.value.copy(resolveColor(container, color))
  ...
  useEffect(() => {
    ...
    const syncColor = () => {
      ...
      if (state) state.uniforms.uColor.value.copy(resolveColor(container, color))
    }

    syncColor()
    const colorObserver = new MutationObserver(syncColor)
    ...
    colorObserver.observe(container, observerOptions)
  ...
  return (
    <div
      ref={containerRef}
      className={cn("pixel-blast-container", className)}
      style={style}
      aria-label="Pixel blast interactive background"
    />
  )
}

export default BackgroundPixelBlast
