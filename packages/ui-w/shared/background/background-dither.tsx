...
type ThemeColor = RGB | { light: RGB; dark: RGB }
...
function DitheredWaves({
  ...
  useFrame((_, delta) => {
    ...
    if (!disableAnimation) {
      u.time.value += Math.min(delta, 0.1)
    }
...
export function BackgroundDither({
  ...
  className,
  style,
}: BackgroundDitherProps) {
  const theme = useResolvedTheme()
  const resolveColor = (color: ThemeColor): RGB => ("light" in color ? color[theme] : color)

  return (
    <Canvas
      className={cn("dither-container", className)}
    ...
    >
      <DitheredWaves
        ...
        waveColor={resolveColor(waveColor)}
        colorNum={colorNum}
        pixelSize={pixelSize}
